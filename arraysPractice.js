...
// Your job is to write a function named finder that will get a random number (by invoking getRandomArbitrary), then loop through the array (that will be passed in as a parameter) to see if that random number is in the array. If it is, return true, if it's not, return false
  ...
  //Code Here
var a = [0,3,4,5,6,7,9,14,17,24,25,26,29,30];
  
   function finder(a){
    for(var i = 0; i < a.length; i++){
      if(a.includes(getRandomArbitrary(),i)){
        console.log('true')
     return true
    }else{
      console.log('false')
...
function removeItem(myGroceryList, str){
  var i = myGroceryList.indexOf(str);
if(i != -1) {
...
function addItem(myGroceryList, str){
  var j = myGroceryList.length
   if(myGroceryList[j] = str){
...
//emoveItem(myGroceryList, 'chips') --> ['pizza', 'hotpockets', 'MtnDew', 'corndogs'];
...
function maker(start, end, step) {
  ...
  // 
  for (var  arr= []; (end - start) * step > 0; start += step) {
    arr.push(start);
  }
  return arr;
